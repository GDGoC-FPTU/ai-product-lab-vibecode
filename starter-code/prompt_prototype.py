...
def evaluate_prompt(user_input: str) -> str:
    ...
    # Local fallback for demo/offline environments so the prototype can still run.
    if not api_key:
        if any(keyword in lowered_input for keyword in ["2%", "3%", "4%", "critical", "hết pin", "cạn", "pin"]):
            return '{"action": "dispatch_mobile_charger", "reason": "Battery level under critical threshold of 5%. Cannot reach station safely."}'
        return "[DRAFT_ONLY] Tin nhắn chỉ dẫn đã được soạn sẵn và cần phê duyệt bởi điều phối viên trước khi gửi."
...
if __name__ == "__main__":
    ...
    if not api_key:
        print("\033[93m[Warning] GEMINI_API_KEY environment variable is not set; using built-in fallback mode.\033[0m")
