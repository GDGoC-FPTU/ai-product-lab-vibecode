...
# Ensure UTF-8 output on Windows consoles and subprocess captures
...
If the battery is 5 percent or above, you may draft a standard routing guide to the nearest station, ensuring you prefix the text with '[DRAFT_ONLY] '.
"""


def _detect_critical_battery(user_input: str) -> bool:
    """Heuristic check for critical battery based on explicit percentages in the user input."""
    lower_text = user_input.lower()
    if "pin" not in lower_text and "battery" not in lower_text and "%" not in lower_text:
        return False

    matches = re.findall(r"(\d+(?:\.\d+)?)\s*%", lower_text)
    if not matches:
        return False
...
def _fallback_response(user_input: str) -> str:
    """Deterministic local fallback used when no API key or SDK is available."""
    if _detect_critical_battery(user_input):
        return '{"action": "dispatch_mobile_charger", "reason": "Battery level under critical threshold of 5%. Cannot reach station safely."}'
    return "[DRAFT_ONLY] Tin nhắn draft: Hãy giữ xe an toàn, kiểm tra trạm sạc gần nhất và chờ dispatcher phê duyệt trước khi gửi."
...
if __name__ == "__main__":
    api_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
    if not api_key:
        print("\033[93m[Info] GEMINI_API_KEY not set; using deterministic local fallback for safe testing.\033[0m")
